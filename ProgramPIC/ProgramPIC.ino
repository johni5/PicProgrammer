...
#define PIN_MCLR        A6                // A1 0: MCLR is VPP voltage, 1: Reset PIC
#define PIN_ACTIVITY    LED_BUILTIN       // A5 LED that indicates read/write activity
#define PIN_VDD         3                 // Controls the power to the PIC
...
#define DELAY_TDLY2     1       // 1 Delay between commands or data
#define DELAY_TDLY3     1       // Delay until data bit read will be valid

#define DELAY_TPROG_MS     4    // 4000 Time for a program memory write to complete
#define DELAY_TDPROG_MS    6    // 6000 Time for a data memory write to complete
#define DELAY_TERA_MS      6    // 6000 Time for a word erase to complete
#define DELAY_TPROG5_MS    1    // Time for program write on FLASH5 systems
#define DELAY_TFULLERA_MS  50   // Time for a full chip erase
#define DELAY_TFULL84_MS   20   // Intermediate wait for PIC16F84/PIC16F84A
...
const char s_pic16f873[]  PROGMEM = "pic16f873";
...
/*
    name | DeviceID |  progSize | confStart | dataStart | configSize | dataSize | reservedWords | configSave | progFlashType | dataFlashType | progEntryMode
*/
struct deviceInfo const devices[] PROGMEM = {
  ...
  {s_pic16f873,  0x0960, 4096, 0x2000, 0x2100, 8, 128, 0, 0, FLASH7, EEPROM, Vdd_first},
...
void cmdErase(const char *args)
{
  ...
  bool eeprom = matchString(s_eeprom, args, len);
  ...
  switch (progFlashType) {
    ...
    case FLASH7:
      ...
      sendSimpleCommand(1);    // Command 1
      sendSimpleCommand(7);    // Command 7
      sendSimpleCommand(CMD_BEGIN_PROGRAM);
      //delay(DELAY_TFULL84_MS);
      delay(8);
      sendSimpleCommand(1);    // Command 1
      sendSimpleCommand(7);    // Command 7
      delay(DELAY_TFULLERA_MS);

      // reset code protection only for 2 attempt
      sendSimpleCommand(1);    // Command 1
      sendSimpleCommand(7);    // Command 7
      sendSimpleCommand(CMD_BEGIN_PROGRAM);
      //delay(DELAY_TFULL84_MS);
    ...
    default:
      ...
      if (eeprom) {
        ...
        //sendWriteCommand(CMD_LOAD_PROGRAM_MEMORY, 0x3FFF);
        state = STATE_CONFIG;
        sendSimpleCommand(1);    // Command 1
        sendSimpleCommand(7);    // Command 7
        sendSimpleCommand(CMD_BEGIN_PROGRAM);
...
const char s_flag[] PROGMEM = "ON";
// test Vdd command.
void cmdTestVdd(const char *args) {
  int len = 0;
  while (args[len] != '\0' && args[len] != ' ' && args[len] != '\t')
    ++len;
  bool flagOn = matchString(s_flag, args, len);
  if (state != STATE_IDLE) {
    Serial.println("ERROR");
    return;
  }
  digitalWrite(PIN_VDD, flagOn ? VDD_ON : VDD_OFF);
  Serial.println("OK");
}

// test Vpp command.
void cmdTestVpp(const char *args) {
  int len = 0;
  while (args[len] != '\0' && args[len] != ' ' && args[len] != '\t')
    ++len;
  bool flagOn = matchString(s_flag, args, len);
  ...
  digitalWrite(PIN_MCLR, flagOn ? MCLR_VPP : MCLR_RESET);
  Serial.println("OK");
}
...
const char s_cmdReadBinaryDesc[] PROGMEM = "Reads program and data words from device memory (binary)";
...
const char s_cmdWriteBinaryDesc[] PROGMEM = "Writes program and data words to device memory (binary)";
const char s_cmdWriteBinaryArgs[] PROGMEM = "STARTADDR";
const char s_cmdErase[] PROGMEM = "ERASE";
const char s_cmdEraseDesc[] PROGMEM = "Erases the contents of program, configuration, and data memory";
...
const char s_cmdTestVdd[] PROGMEM = "TESTVDD";
const char s_cmdTestVddDesc[] PROGMEM = "Test Vdd output";
const char s_cmdTestVpp[] PROGMEM = "TESTVPP";
const char s_cmdTestVppDesc[] PROGMEM = "Test Vpp output";
const command_t commands[] PROGMEM = {
  ...
  {s_cmdTestVdd, cmdTestVdd, s_cmdTestVddDesc, 0},
  {s_cmdTestVpp, cmdTestVpp, s_cmdTestVppDesc, 0},
  {0, 0}
};
...
void beginProgramCycle(unsigned long addr, bool isData)
{
  switch (isData ? dataFlashType : progFlashType) {
    case EEPROM:
      sendSimpleCommand(CMD_BEGIN_PROGRAM);
      delay(DELAY_TDPROG_MS);
      break;
    case FLASH:
    case FLASH7:
      sendSimpleCommand(CMD_BEGIN_PROGRAM);
      delay(DELAY_TPROG_MS + DELAY_TERA_MS);
      break;
    case FLASH4:
      sendSimpleCommand(CMD_BEGIN_PROGRAM);
      delay(DELAY_TPROG_MS);
      break;
    case FLASH5:
      sendSimpleCommand(CMD_BEGIN_PROGRAM_ONLY);
      delay(DELAY_TPROG5_MS);
